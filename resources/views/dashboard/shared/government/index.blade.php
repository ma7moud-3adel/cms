                        ...
                        <form method="POST" action="{{ route('government.store') }}">
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    <div class="col-10">
                                        <h3 class="h1">{{ trans('main.add_gov') }}</h3>
                                        <div class="row">
                                            <div class="markdown text-secondary col">
                                                <label class="form-label">{{ trans('main.gov_name_en') }}</label>
                                                <input type="text" name="name_en" class="form-control"
                                                    placeholder="Enter Government Name" autocomplete="off"
                                                    value="{{ old('name_en') }}">
                                            </div>
                                            <div class="markdown text-secondary col">
                                                <label class="form-label">{{ trans('main.gov_name_ar') }}</label>
                                                <input type="text" name="name_ar" class="form-control"
                                                    placeholder="ادخل اسم المحافظة" autocomplete="off"
                                                    value="{{ old('name_ar') }}">
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{{ trans('main.gov_table') }}</h3>
                        </div>
                        <div class="table-responsive">
                            <table class="table card-table table-vcenter text-nowrap datatable text-center" id="gov-table">
                                <thead>
                                    <tr>
                                        {{-- <th class="w-1">
                                        ...
                                        <th>{{ trans('main.gov_name_en') }}</th>
                                        <th>{{ trans('main.gov_name_ar') }}</th>
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                <a class="btn btn-outline-primary"
                                                    href="{{ route('government.edit', $item->id) }}"
                                                    role="button">{{ trans('main.Edit') }}</a>

                                                <form action="{{ route('government.destroy', $item->id) }}" method="POST"
