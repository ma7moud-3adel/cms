                ...
                <div class="card card-md">
                    ...
                    <form action="{{ route('service.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class=" row">
                                        <div class="mb-5 col-12">
                                            <label for="description_en"
                                                ...
                                                ({{ trans('main.en') }}) </label>
                                            <textarea id="text_area_1" name="description_en" class="form-control" autocomplete="off">{{ $service->description_en }}</textarea>
                                        </div>
                                        <div class="mb-5 col-12">
                                            <label for="description_ar"
                                                class="form-label required">{{ trans('main.description') }}
                                                ({{ trans('main.ar') }}) </label>
                                            <textarea id="text_area_2" name="description_ar" class="form-control" autocomplete="off">{{ $service->description_ar }}</textarea>
                                        </div>
                                    </div>
                                    <div class=" row">
                                        <div class="mb-3 col">
                                        ...
                                        <div class="mb-3 col ">
                                            <label for="service_group_id"
                                                class="form-label required">{{ trans('main.service_group') }}</label>
                                            <select name="service_group_id" class="form-control">
                                                @foreach ($serviceGroups as $serviceGroup)
                                                    @if ($service->service_group_id == $serviceGroup->id)
                                                        <option selected value="{{ $serviceGroup->id }}">
                                                            {{ $serviceGroup->name_en }}</option>
                                                    @else
                        ...
                        <div class="card-footer text-end">
                            ...
                            <a href="{{ route('service.index') }}" class="btn btn-secondary" role="button"
